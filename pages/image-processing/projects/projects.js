  ...
  const listProject = async () => {
    try {
      const res = await axios.get(
        `http://localhost:4000/api/v1/users/imagelist/${user._id}`,
      )
      ...
      <main className={styles.main}>
        ...
        <div className={styles.grid}>
          {projectData &&  projectData.map((item) => {
            return (
              <Link href={`/image-processing/project-detail/project?=${item.project_id}`} className={styles.card} key={item.name}>
